...
#### Basado en código de David Zumbach https://github.com/zumbov2
#### Land doesnt vote, people do https://github.com/zumbov2/votemapswitzerland
#### 15/08/2021



# Load packages ------------------------------------------------------------------------
library(dplyr) # A Grammar of Data Manipulation, CRAN v1.0.7
...
library(tibble) # Simple Data Frames, CRAN v3.1.3
library(purrr) # Functional Programming Tools, CRAN v0.3.4
library(transformr) # Polygon and Path Transformations, [github::thomasp85/transformr] v0.1.3.9000
library(tweenr) # Interpolate Data for Smooth Animations, CRAN v1.0.1
library(particles) # A Graph Based Particle Simulator Based on D3-Force, CRAN v0.2.2
library(igraph) # Network Analysis and Visualization, CRAN v1.2.6
library(tidygraph) # A Tidy API for Graph Manipulation, CRAN v1.2.0
...
# Geometrías de Departamentos de Argentina

gd_mun <-  get_geo(geo = "ARGENTINA") %>%  # division politica Argentina a nivel departamento
  add_geo_codes() %>%  # Agrega IDs geografico electorales para unir a datos de votaciones
  mutate(mun_id = paste0(codprov, coddepto)) %>%  # genero ID unico de deptos (provincia + depto)
  filter(!is.na(codprov)) %>% 
  st_transform(crs = 3857)

gd_nat <- gd_mun %>% summarise(geometry = st_union(geometry)) %>% 
  mutate(mun_id = 1) 
...
# Datos electorales con {electorAr}
vd <- get_election_data(district = "arg", # Datos electorales de Argentina
                              category = "presi",
                              round = "balota",
                              year = 2015, 
                              level = "departamento", long = F) %>% 
  rename(fdt = `0131`, 
         cambiemos = `0135`) %>% 
  mutate(votos_positivos = fdt + cambiemos, 
         fdt_pct = fdt / votos_positivos * 100, 
         cambiemos_pct = cambiemos / votos_positivos *100, 
         mun_id = paste0(codprov, coddepto)) 


# Wrangle missing data 
ausentes <- anti_join(vd,gd_mun,   by = "mun_id") %>% 
  pull(mun_id)

vd <- vd %>% 
  filter(!mun_id %in% ausentes)
...
ranking_area <- tibble(
  ...
  area = st_area(gd_mun) / 1000 ^ 2 #Metros cuadras
) %>% 
  left_join(vd %>% select(mun_id, cambiemos_pct), by = "mun_id") %>% 
  mutate(yes = ifelse(cambiemos_pct >= 50, 1, 0)) %>% 
  group_by(yes) %>% 
  mutate(rank_area = rank(1/area, ties.method = "first")) %>% 
  ungroup() %>% 
  filter(!is.na(cambiemos_pct))
...
# Define grid limits 
grid_X_lim_yes <- c(grid_lim$X_min - .5*grid_lim$X_min, 
                    grid_lim$X_min + (grid_lim$X_max - grid_lim$X_min)/2 - .1*grid_lim$X_min)
...
sum_padding <- function(..., padding = 40000) sum(...) + padding 
...
get_new_positions <- function(row_id, data, grid_X_lim, grid_Y_lim, padding_edge = 50000) {
...
# Define grid limits 
grid_X_lim_no <- c(grid_X_lim_yes[2] + .1*grid_X_lim_yes[2],
                   grid_lim$X_max + .5*grid_X_lim_yes[2])
...
step3_centers <- step2 %>% 
  ...
  left_join(vd %>% select(mun_id, votos_positivos), by = "mun_id")




# Define conversion factor votes to area
votes_total <- sum(vd$votos_positivos[vd$mun_id %in% step3_centers$mun_id])
...
votos_final <- vd %>% 
  ungroup() %>% 
  summarise(across(.cols = c(fdt, cambiemos), .fns = ~ sum(.)/votes_total*100))

# Define circle radius from vote
step3_circle_data <- step3_centers %>% 
  mutate(radius = as.numeric(sqrt(conv_fac * votos_positivos / pi)))
...
get_bubble_positions <- function(data, bubble_center_x, bubble_center_y, spacing = 10000) {
...
# Split circles into Cambiemos and FPV parts and arrange as bubble plots ----------------------

# Cambiemos votes from municipalities with Cambiemos majority
yy_radius <- step4_yes %>% 
  left_join(vd %>% select(mun_id, cambiemos), by = "mun_id") %>% 
  mutate(radius = as.numeric(sqrt(conv_fac * cambiemos / pi)))
...
# FPV votes from municipalities with Cambiemos majority
yn_radius <- step4_yes %>% 
  left_join(vd %>% select(mun_id, fdt), by = "mun_id") %>% 
  mutate(radius = as.numeric(sqrt(conv_fac * fdt / pi))) %>% 
...
# FPV votes from municipalities with FPV majority
nn_radius <- step4_no %>% 
  left_join(vd %>% select(mun_id, fdt), by = "mun_id") %>% 
  mutate(radius = as.numeric(sqrt(conv_fac * fdt / pi)))
...
# Cambiemos votes from municipalities with FPV majority
ny_radius <- step4_no %>% 
  left_join(vd %>% select(mun_id, cambiemos), by = "mun_id") %>% 
  mutate(radius = as.numeric(sqrt(conv_fac * cambiemos / pi))) %>% 
  mutate(mun_id = paste0(mun_id, "_cp"))
...
td_def <- td3 %>% 
  ...
  left_join(vd %>% select(mun_id, cambiemos_pct), by = c("mun_id2" = "mun_id")) %>% 
  select(-mun_id2) %>% 
  mutate(cambiemos_pct = ifelse(str_detect(mun_id, "_cp"), 100 - cambiemos_pct, cambiemos_pct)) %>%
  
  # To equalise colours of all yes/no bubbles -> Visually, I prefer the less accurate bubble chart with gradations. 
  # mutate(cambiemos_pct = case_when(
  # .frame <= tot - t3 - p4 ~ cambiemos_pct,
  # .frame > tot - t3 - p4 & !str_detect(mun_id, "_cp") & cambiemos_pct < 50 ~ 19,
  # .frame > tot - t3 - p4 & !str_detect(mun_id, "_cp") & cambiemos_pct >= 50 ~ 80,
  # .frame > tot - t3 - p4 & str_detect(mun_id, "_cp") & cambiemos_pct <= 50 ~ 19,
  # .frame > tot - t3 - p4 & str_detect(mun_id, "_cp") & cambiemos_pct >= 50 ~ 80
  # )) %>% 
  
  # Prevent blue bubbles in no bubble plot (== 50%) 
mutate(cambiemos_pct = ifelse(
  .frame > tot - t3 - p4 & 
    str_detect(mun_id, "_cp") & 
    cambiemos_pct == 50, 49, cambiemos_pct
)) %>% 
  mutate(
    stimmen = factor(case_when(
      cambiemos_pct < 20 ~ "",
      cambiemos_pct >= 20 & cambiemos_pct < 30 ~ "20", 
      cambiemos_pct >= 30 & cambiemos_pct < 40 ~ "30",
      cambiemos_pct >= 40 & cambiemos_pct < 50 ~ "40",
      cambiemos_pct >= 50 & cambiemos_pct < 60 ~ "50",
      cambiemos_pct >= 60 & cambiemos_pct < 70 ~ "60",
      cambiemos_pct >= 70 & cambiemos_pct < 80 ~ "70",
      cambiemos_pct >= 80 ~ "80"
...
plot_data <- function(data, pos, xlim, ylim) {
  ...
  # Title
  subtitle <- case_when(
    pos <= p1 + t1 + p2 + t2/2 ~ "2015: balotaje presidencial de Argentina",
    pos > p1 + t1 + p2  + t2/2 & pos <= p1 + t1 + p2 + t2 + p3 + t3/2 ~ "La tierra no vota...",
    pos > p1 + t1 + p2 + t2 + p3 + t3/2 ~ "...la gente lo hace"
  )
  
  # Caption
  caption <- case_when(
    pos <= p1 + t1/2 ~ "Resultados a nivel DEPARTAMENTO",
    pos > p1 + t1/2 & pos <= p1 + t1 + p2 + t2/2 ~ "DEPARTAMENTOS ordenados por voto mayoritario y área geográfica",
    pos > p1 + t1 + p2 + t2/2 & pos <= p1 + t1 + p2 + t2 + p3 + t3/2  ~ "DEPARTAMENTOS ordenados por voto mayoritario y cantidad de votos positivos",
    pos > p1 + t1 + p2 + t2 + p3 + t3/2  & pos < tot - p4 + 20 ~ "% votos por fuerza para todos los departamentos de Argentina",
    pos >= tot - p4 + 20 ~ "FPV: 48.6% - Cambiemos: 51.4% "
  )
  ...
  p <- ggplot(data$geometry) +
    ...
    scale_fill_manual(
      values = c(
        "#105182", "#1a7bc5", "#42a2f1", "#91cdff",
        "#fbf5ab", "#faf393", "#f8ed62", "#f5e519"
      ),
      drop = F,
      name = "% votos positivos Cambiemos vs FPV",
      guide = guide_legend(
    ...
    labs(
      title = subtitle,
      subtitle = caption, 
      caption = "Por @TuQmano con datos disponibles en {polArverse}.\n Basado en código de @DavidZumbach") +
    theme_ipsum_rc() +
    theme(
